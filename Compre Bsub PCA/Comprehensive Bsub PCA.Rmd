...
plotIndiv(MyResult.sipca, comp = c(1,2), legend = TRUE, title = 'B. subtilis - Media vs Pellet: IPCA',
          group = BsubMP$Label, legend.title = 'Treatment',
          ellipse = TRUE, style = "ggplot2", ind.names = FALSE)
...
X <- BsubFG[,3:ncol(BsubFG)]
MyResult.sipca <- sipca(X, ncomp = 3, mode="deflation", keepX = c(50,50,50))
plotIndiv(MyResult.sipca, comp = c(1,2), legend = TRUE, title = 'B. subtilis - Flight vs Ground: IPCA',
          group = BsubFG$Label, legend.title = 'Treatment',
          ellipse = TRUE, style = "ggplot2", ind.names = FALSE)
plotIndiv(MyResult.sipca, comp = c(1,2,3), legend = TRUE, title = 'B. subtilis - Flight vs Ground: IPCA',
          group = BsubFG$Label, legend.title = 'Treatment',
          ellipse = TRUE, style = "3d", ind.names = FALSE)
plotVar(MyResult.sipca, cutoff = 0.8)
MyResult.sipca$kurtosis 
...
anova_data <- BsubFG[,c("Label", "110647", "5085", "160843", "101299", "O-acetylserine", "stearic acid")]
library(tidyverse)
anova_data %>% pivot_longer(-Label, names_to = "Metabolite", values_to = "value") %>%
  mutate(Metabolite = factor(Metabolite, levels = c("110647", "5085", "160843", "101299", "O-acetylserine", "stearic acid"))) %>%
  ggplot(aes(x = Label, y = value, color = Metabolite)) +
  geom_boxplot() +
  facet_wrap(~Metabolite)
