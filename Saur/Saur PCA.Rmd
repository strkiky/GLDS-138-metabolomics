...
# straight to tuning                    ### this analysis seems to be pointless
...
ncomp
tune.spls.MAE$choice.keepX # 2, 20, 2, 2
...
pls_metab <- SaurFG[,c("Label", "n-acetyl-d-hexosamine", "146563", "glycolic acid", "glycerol-alpha-phosphate", "21683", "3-phenyllactic acid")]
library(tidyverse)
pls_metab %>% pivot_longer(-Label, names_to = "Metabolite", values_to = "value") %>%
  mutate(Metabolite = factor(Metabolite, levels = c("n-acetyl-d-hexosamine", "146563", "glycolic acid", "glycerol-alpha-phosphate", "21683", "3-phenyllactic acid"))) %>%
  ggplot(aes(x = Label, y = value, color = Metabolite)) +
  geom_boxplot() +
  facet_wrap(~Metabolite)

pls_lmetab <- Ldata[,c("Label", "V442", "V453", "V583", "V228", "V800", "V894")]
pls_lmetab %>% pivot_longer(-Label, names_to = "Metabolite", values_to = "value") %>%
  mutate(Metabolite = factor(Metabolite, levels = c("V442", "V453", "V583", "V228", "V800", "V894"))) %>%
  ggplot(aes(x = Label, y = value, color = Metabolite)) +
  geom_boxplot() +
  facet_wrap(~Metabolite)
```
...
list.keepX <- list(metabolite = c(10,5), lipid = c(10,5))
...
plotIndiv(MyResult.diablo,
          ...
          title = "Spaceflight S. aureus with DIABLO") ## sample plot
plotVar(MyResult.diablo, var.names = c(TRUE, FALSE),
        legend = TRUE, pch = c(16, 2))
...
cimDiablo(MyResult.diablo, comp = 1,
          color.blocks = c('darkorchid', 'brown1'),  margin=c(9,20), 
