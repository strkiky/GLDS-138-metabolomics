...
splsda_metab <- SaurFG[,c("Label", "glycolic acid", "217691", "3-hydroxypropionic acid", "4721", "methionine")]
library(tidyverse)
splsda_metab %>% pivot_longer(-Label, names_to = "Metabolite", values_to = "value") %>%
  mutate(Metabolite = factor(Metabolite, levels = c("glycolic acid", "217691", "3-hydroxypropionic acid", "4721", "methionine"))) %>%
  ggplot(aes(x = Label, y = value, color = Metabolite)) +
  geom_boxplot() +
  facet_wrap(~Metabolite)
```
