...
heatmap(sol$U[[1]],col=hmcols,labRow=NA,labCol=NA,ColSideCol=cols[ty])
...
plot(cbass_fit, k.col = 8, heatcol.label.cex = 0.6, heatrow.label.cex = 0.2) # can't see the long names
...
cbass_metab <- SaurFG[,c("Label", "133605", "3122", "4609", "5263", "236845", "192876")]
library(tidyverse)
cbass_metab %>% pivot_longer(-Label, names_to = "Metabolite", values_to = "value") %>%
  mutate(Metabolite = factor(Metabolite, levels = c("Label", "133605", "3122", "4609", "5263", "236845", "192876"))) %>%
  ggplot(aes(x = Label, y = value, color = Metabolite)) +
  geom_boxplot() +
  facet_wrap(~Metabolite)
